...
def tournerPasFixe(direction, puissance, degrees):
    b = 0 #2.5 pour 80 et 1.8 pour 100
    a = 1.2 
    ...
    if direction == "droite":
        coefL = 1
        coefR = -MIN_PUISSANCE/MAX_PUISSANCE
    else:
        coefL = -MIN_PUISSANCE/MAX_PUISSANCE
        coefR = 1
    ...
    write_order(serial_file, Order.MOTOR)
    write_i8(serial_file, int(puissance*coefR)) #valeur moteur droit
    write_i8(serial_file, int(puissance*coefL)) #valeur moteur gauche
