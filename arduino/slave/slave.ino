...
Metro sweepServoMetro = Metro(20);

Servo frontServo, backServo;    // create servo objects
#define  frontPin 9
#define  backPin 8
int pos = INITIAL_THETA;    // initial position of the servos
int sweepFlag = 1;
...
int8_t DistanceLimitee(){
  return int8_t(min(MeasureDistance(), __INT8_MAX__));
}
...
void servoSweep(){
  if(not clockwise){
    if(pos>=THETA_MIN && pos<=THETA_MAX){
      pos++;                            // in steps of 1 degree
      frontservo.write(pos);               // tell servo to go to position in variable 'pos'
    }
    if(pos>=THETA_MAX)  clockwise = true; // change direction
  }else{
    if(pos>=THETA_MIN && pos<=THETA_MAX){
      pos--;
      frontservo.write(pos);
    }
    if(pos<=THETA_MIN)  clockwise = false;
  }
}
